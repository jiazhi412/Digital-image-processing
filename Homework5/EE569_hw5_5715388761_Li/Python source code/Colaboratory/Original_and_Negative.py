...
# Combine original and negative images together
X_train_comb = np.concatenate((X_train, X_train_negative))
X_test_comb = np.concatenate((X_test, X_test_negative))
Y_train_comb = np.concatenate((Y_train, Y_train))
Y_test_comb = np.concatenate((Y_test, Y_test))
print(X_train_comb.shape)
print(X_test_comb.shape)
print(Y_train_comb.shape)
print(Y_test_comb.shape)
...
# Show combination image by opencv
images = []
img = []

for i in range(X_train_comb.shape[0]):
    im = X_train_comb[i]
    im = im.reshape(28, 28)
    if (img == []):
        img = im
...
# convert class vectors to binary class matrices (single)
Y_train = np_utils.to_categorical(Y_train, NB_CLASSES)
Y_test = np_utils.to_categorical(Y_test, NB_CLASSES)

# normalize the combination data
X_train_comb = X_train_comb.astype("float32")  # change the type of ndarray
X_test_comb = X_test_comb.astype("float32")
X_train_comb /= 255
X_test_comb /= 255
X_train_comb = X_train_comb.reshape(X_train_comb.shape[0], 1, IMG_ROWS, IMG_COLS)
X_test_comb = X_test_comb.reshape(X_test_comb.shape[0], 1, IMG_ROWS, IMG_COLS)

# convert class vectors to binary class matrices (double)
Y_train_comb = np_utils.to_categorical(Y_train_comb, NB_CLASSES)
Y_test_comb = np_utils.to_categorical(Y_test_comb, NB_CLASSES)

# init the optimizer and model
model = LeNet.createLeNet(input_shape=INPUT_SHAPE, nb_class=NB_CLASSES)
model.compile(loss="categorical_crossentropy", optimizer=OPTIMIZER, metrics=["accuracy"])

history = model.fit(X_train_comb, Y_train_comb,
                    batch_size=BATCH_SIZE,
                    epochs=NB_EPOCH,
                    verbose=VERBOSE,
                    validation_split=VALIDATION_SPLIT)

# Evaluate the model by original images
score = model.evaluate(X_test, Y_test, verbose=VERBOSE)
print("Test score by original images:", score[0])
print("Test accuracy by original images:", score[1])

# Evaluate the model by negative images
...
# Evaluate the result
score = model.evaluate(X_test_comb, Y_test_comb, verbose=VERBOSE)
